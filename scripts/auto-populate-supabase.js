...
/**
 * Auto-populate Supabase com todos os 85+ produtos
 * Usa Supabase REST API - sem dependências extras
 */
...
async function populateSupabase() {
  const supabaseUrl = process.env.VITE_SUPABASE_URL || 'https://aspbntijurqpeskempue.supabase.co';
  const supabaseKey = process.env.SUPABASE_ANON_KEY;

  if (!supabaseKey) {
    console.error('❌ ERRO: Chave Supabase não encontrada em SUPABASE_ANON_KEY');
    process.exit(1);
  }
  ...
  try {
    // Primeiro, deletar todos os registros antigos
    console.log('🧹 Limpando dados antigos...');
    let deleteResponse = await fetch(`${supabaseUrl}/rest/v1/cardapio?select=id`, {
      method: 'DELETE',
      headers: {
        'apikey': supabaseKey,
    ...
    if (!deleteResponse.ok) {
      console.log('⚠️  Aviso ao deletar dados antigos (pode ser normal se tabela está vazia)');
    } else {
      console.log('✅ Dados antigos removidos');
    }

    // Inserir produtos em lotes
    console.log(`📝 Inserindo ${cardapioData.length} produtos em lotes...`);
    const loteSize = 10;
    let inserted = 0;
    ...
    for (let i = 0; i < cardapioData.length; i += loteSize) {
      ...
      if (!response.ok) {
        ...
        console.error(`❌ ERRO ao inserir lote ${Math.floor(i/loteSize) + 1}:`, error);
        process.exit(1);
      }

      inserted += lote.length;
      console.log(`✅ ${inserted}/${cardapioData.length} produtos inseridos...`);
    }

    console.log(`\n✅ SUCESSO! ${cardapioData.length} produtos inseridos no Supabase!`);

    // Validação
    console.log('\n📊 Validando dados inseridos...');
    const validationResponse = await fetch(
      `${supabaseUrl}/rest/v1/cardapio?select=categoria,count()`,
      {
        headers: {
          'apikey': supabaseKey,
          'Authorization': `Bearer ${supabaseKey}`,
    ...
    if (validationResponse.ok) {
      ...
      data.forEach(row => {
        if (row.categoria) {
          console.log(`${row.categoria.padEnd(20)} | ${row.count} produtos`);
          totalGeral += parseInt(row.count);
        }
      });
      console.log('==========================');
    ...
    console.log('🎉 Seu cardápio está pronto! 85+ produtos no Supabase!');
